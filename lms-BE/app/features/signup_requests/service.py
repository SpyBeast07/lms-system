...
from .schemas import SignupRequestCreate, SignupApprovalRequest, PaginatedSignupRequests
...
async def _notify_admins(db: AsyncSession, request: SignupRequest) -> None:
...
async def create_signup_request(
    ...
    if existing_req:
        ...
        await _notify_admins(db, existing_req)
        return existing_req
    ...
    await _notify_admins(db, request)
    return request
